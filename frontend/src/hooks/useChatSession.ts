 ...
 *  - НЕ показывает предупреждение про код восстановления. Тексты лежат в
 *    `messages/*.json` (`chat.recovery_warning_*`), показывает их отображение
 *    (Задача 7). Здесь только признак «код есть и его надо показать».
 */
