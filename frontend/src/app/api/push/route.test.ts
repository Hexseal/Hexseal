...
describe('К-2: /api/push требует доказательства, а ссылку не пересылает', () => {
  ...
  it('ЗАМЕР: сколько запросов рода «спор» пролезает с ОДНОГО источника за окно', async () => {
    // Каждый пролезший запрос — это потенциальное обращение релеера к узлу
    // цепи (по выдуманному адресу кэш не помогает). Число ниже — верхняя
    // граница усиления с одного источника; общий потолок на стороне релеера
    // считает уже поверх неё.
    let через = 0;
    for (let i = 0; i < 400; i++) {
      const deal = `0x${String(i).padStart(40, 'e')}`;
      const res = await POST(req({ to: VICTIM, kind: 'dispute', deal }, {
        'x-forwarded-for': '198.51.100.5',
      }));
      if (res.status !== 429) через++;
    }
    // eslint-disable-next-line no-console
    console.log(`[замер] 400 запросов «спор» с одного источника → дошло до релеера: ${через}`);
    expect(через).toBeLessThanOrEqual(120);
    expect(fetchMock.mock.calls.length).toBe(через);
  });
