 ...
 * ─── ЧТО СКАЗАЛ ВЛАДЕЛЕЦ ────────────────────────────────────────────────────
 *
 * По смыслу: список подгружается рывками — на каждом фоновом обновлении весь
 * фронт скидывается до скелетона, хотя так быть не должно: максимум «+чат», а
 * не сброс до заготовок и обратно. На такие прыжки невозможно смотреть.
 *
...
describe('пустое превью не выдаётся за «сообщений нет»', () => {
  ...
  it('«сообщений нет» стоит РОВНО ОДИН раз — у той строки, где это правда', async () => {
    ...
    // стоит там, где надо.
    setConv({ conversations: [
      { peerAddress: PEER,  lastText: 'наш ответ', lastAt: LONG_AGO, lastFromMe: true,  preview: 'text' },
      { peerAddress: PEER2, lastText: '', lastAt: LONG_AGO, lastFromMe: false, preview: 'pending' },
      { peerAddress: '0x6666666666666666666666666666666666666666', lastText: '', lastAt: LONG_AGO, lastFromMe: false, preview: 'none' },
    ] });
    const html = await renderList();
  ...
  it('НАШЕ последнее сообщение показано текстом с пометкой «Вы»', async () => {
    // ⚠️ Владелец на живом телефоне: «он не отображает сообщение отправляющего».
    // Подпись «вы написали последним» была не починкой — текст у нас есть.
    setConv({ conversations: [
      { peerAddress: PEER, lastText: 'наше последнее слово', lastAt: LONG_AGO, lastFromMe: true, preview: 'text' },
    ] });
    const html = await renderList();
    expect(html, 'своего сообщения в списке нет').toContain('наше последнее слово');
    expect(html, 'пометки «Вы» нет — не понять, кто написал')
      .toContain(`${translate('chat.preview_you')}: наше последнее слово`);
  });

  it('ЧУЖОЕ последнее сообщение — без пометки', async () => {
    // Замок, который горит всегда, — не замок.
    setConv({ conversations: [
      { peerAddress: PEER, lastText: 'их слово', lastAt: LONG_AGO, lastFromMe: false, preview: 'text' },
    ] });
    const html = await renderList();
    expect(html).toContain('их слово');
    expect(html).not.toContain(`${translate('chat.preview_you')}: их слово`);
  });
});
