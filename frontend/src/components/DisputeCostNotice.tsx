  ...
  return (
    <div className="rounded-[14px] border border-amber-400/25 bg-amber-400/[0.05] px-3 py-2.5 space-y-1.5">
      ...
      {/* Средство от исхода, о котором предупреждает строка выше, теперь
          ...
          Строка НЕ гейтится на `fee * 0.8 < arbiterFloor`, хотя доплата на
          крупном котле и правда всегда нулевая (порог $10 при доле 80% от 3%
          сбора — это котёл от ~$417, выше которого кнопки не будет никогда).
          Три причины, все против гейта:
            • arbiterFloor живёт в ДИАМОНДЕ (getArbiterFloor), а этот компонент
              читает только Agreement по одному адресу. Четвёртое чтение — это
              второй адрес, второй ABI и второе состояние загрузки;
            • охранник на :70-95 прячет ВЕСЬ блок, пока хоть одно чтение не
              доехало. Затащив туда порог, мы дали бы нечитаемому порогу гасить
              предупреждение о сборе и о дележе — те два факта, ради которых
              компонент и существует. Не затащив — завели бы отдельное решение
              fail-open/fail-closed на четвёртый источник;
            • ARBITER_SHARE_BPS во фронте — зеркало константы фасета, и
              lib/disputeBounty.ts прямо предупреждает о дрейфе. Гейтить на
              зеркале ВИДИМОСТЬ предупреждения значит сделать так, что расхождение
              с контрактом молча прячет средство.
          Поэтому условность переехала в сам текст: строка говорит, что вариант
          появляется, только когда сбора самого по себе мало. Цена — одна фраза
          вместо блока, который умеет погасить два других предупреждения. */}
      <p className="text-[11px] leading-relaxed text-white/45 pl-[22px]">
