 ...
 */
/**
 * ⚠️ ЧТО ИЗ ТРЕТЬЕЙ СТРОКИ БЕЙДЖА ПРАВДА СЕГОДНЯ (сверено на сдаче Выкатки 1 4в-2).
 *
 * `chat.privacy_badge_dispute` обещает: «предъявить переписку арбитру может
 * каждая из сторон — со своего устройства». До 4в-2 сбылась только половина
 * обещания, и это было записано здесь же. Теперь сбылись обе:
 *
 ...
 *  - **Дорога есть.** Сторона собирает предъявление и кладёт мешок в ящик
 *    спора, арбитр открывает его своим ключом из цепи и читает. Замок —
 *    `badgePromiseBacked.test.ts`: он краснеет, если у сборки, печати или
 *    чтения предъявления снова не останется ни одного боевого вызывающего.
 *    Он сторожит УПОТРЕБЛЕНИЕ библиотеки, а не наличие этого абзаца — но
 *    считает ОДНО звено, а не всю цепь до экрана; что кнопка дошла до глаз,
 *    доказывают тесты Задач 6 и 7, каждый на своей стороне.
 *
 * ⚠️ И ЧЕГО ЭТА СТРОКА НЕ ОБЕЩАЕТ — чтобы никто не дочитал лишнего:
 *
 *  - **черновик предъявления живёт на ОДНОМ устройстве** (IndexedDB,
 *    `presentationDraft.ts`; открытая находка 50.3). Незаконченное
 *    предъявление на втором устройстве невидимо — там его собирают заново.
 *    Сама переписка при этом достижима отовсюду, где есть ключ чата: разница
 *    именно в черновике;
 *  - **отпечатка предъявления в цепи в Выкатке 1 нет** (§2.11 замысла).
 *    Факт предъявления держится нашим складом, и «сторона предъявила раньше,
 *    чем арбитр записал молчание» цепью пока не доказывается ничем;
 *  - слов **«арбитр прочитал»** здесь нет и не будет: склад знает «забрал» —
 *    это про байты, а не про глаза.
 */
