...
describe('дверь к коду восстановления видна именно тогда, когда сеанса нет', () => {
  ...
  /* ═══════ ДВЕРЬ ОБЯЗАНА СКАЗАТЬ, КОМУ ОНА ══════════════════════════════
     ⚠️ ЗАМЕР С ЖИВОГО ТЕЛЕФОНА (Redmi по кабелю, 9 августа). Адрес
     `0xe21e360c…`, кода на цепи нет, ключа на устройстве нет, ключ в
     справочнике есть. На экране две кнопки: «Включить мессенджер» и
     «Восстановить по коду». Владелец прочёл это как «волетконнект просит
     восстановиться по коду» — то есть как требование к себе.

     Дороги по коду у обычного кошелька НЕ СУЩЕСТВУЕТ: `openSessionFromRecoveryCode`
     отказывает кодом `recovery_not_applicable`. Отказ честный и слова у него
     верные — но человек читает их ПОСЛЕ того, как ввёл двенадцать слов. Дверь
     обязана назвать себя ДО того, как в неё пошли. */
  it.each([
    ['session_absent'], ['storage_read_failed'], ['storage_blocked'], [null],
  ])('причина «%s»: рядом с дверью сказано, кому нужен код', async (code) => {
    const html = await renderShared(code as string | null);
    expect(html, 'дверь по коду молчит о том, кому она').toContain(translate('chat.restore_who_for'));
  });

  it('ключа на устройстве нет — сказано, что обычному кошельку хватит подписи', async () => {
    // Ровно состояние с прибора: ключ в справочнике есть, на устройстве нет.
    // Для обычного кошелька это «подпишите снова — вернётся тот же ключ», а не
    // «восстанавливайтесь по коду».
    const html = await renderShared('session_absent');
    expect(html, 'экран не сказал, что ключ вернётся по подписи')
      .toContain(translate('chat.messaging_off_same_key'));
  });

  it('у беды с хранилищем этой надписи НЕТ — она не про подпись', async () => {
    // Замок, который горит всегда, — не замок. «Ключ вернётся по подписи»
    // верно там, где ключа нет вовсе; занятая соседней вкладкой база — другая
    // беда, и обещать там возврат ключа значило бы врать.
    const html = await renderShared('storage_blocked');
    expect(html).not.toContain(translate('chat.messaging_off_same_key'));
  });
});
