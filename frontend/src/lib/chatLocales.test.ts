 ...
 *
 * ⚠️ ЛОКАЛЕЙ ЧЕТЫРНАДЦАТЬ, И КАТАЛОГ ТЕПЕРЬ РОВНО ИМ И РАВЕН. До 14 августа
 * 2026 рядом лежал пятнадцатый файл, `zh.json`, — сирота от старого набора: в
 * `i18n/config.ts` его не было, `LocaleProvider` его не грузил, ключей он
 * отставал на три сотни. Удалён волной правок общего ревью; расхождение
 * «каталог против списка» с тех пор стережётся замком в конце файла, потому что
 * публикуется каталог целиком, а замки ходили по списку.
 */
...
/** Четырнадцать локалей приложения — те же, что в `i18n/config.ts`. */
const LOCALES = [
...
describe('тексты пересадки — 14 локалей', () => {
  ...
  it('каталог переводов и список локалей — одно и то же, сирот больше нет', () => {
    // ⚠️ ЗАМЕНИЛ СОБОЙ ЗАМОК «zh.json не тронут» (волна правок общего ревью,
    // правка 4). `zh.json` был сиротой: 301 ключ позади живых локалей, вне
    // `i18n/config.ts`, `LocaleProvider` его не грузил — пятнадцатый файл,
    // который выглядел локалью и ею не был. Каталог при этом ПУБЛИКУЕТСЯ
    // целиком, и один раз это уже стоило пяти лишних дней жизни мёртвому
    // адресу `0xF00CC718…`: замки ходили по списку, а файл лежал в каталоге.
    // Теперь стережётся само расхождение, а не конкретная сирота.
...
describe('справка и бейджи не врут после сноса XMTP (аудит текстов)', () => {
  ...
  it('каждый ключ chat.* есть во всех 14 локалях и не пуст', () => {
...
describe('адрес контракта не вписан в переводы руками', () => {
  ...
  // ⚠️ ХОДИМ ПО КАТАЛОГУ, А НЕ ПО СПИСКУ `LOCALES`, И ПОСЛЕ СНОСА СИРОТЫ ТОЖЕ.
  // Список — это то, что грузит приложение; публикуется же каталог целиком.
  // Замер 12 августа 2026: `zh.json` в приложение подключён не был, а мёртвый
  // адрес `0xF00CC718…` пролежал в нём ещё пять дней после того, как его
  // убрали из четырнадцати подключённых, — потому что оба замка выше ходили по
  // списку. Сироту снесли 14 августа (волна правок общего ревью), но привычку
  // ходить по каталогу оставили: она дешевле, чем второй такой же промах.
