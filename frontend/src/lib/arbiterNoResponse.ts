 ...
 *
 * ⚠️ ПОРЯДОК СОСТОЯНИЙ ПОВТОРЯЕТ ПОРЯДОК ПРОВЕРОК В КОНТРАКТЕ, И ЭТО ПРЕДМЕТ
 * ЗАДАЧИ, А НЕ ПЕДАНТИЗМ. `recordNoResponse` (src/facets/ArbiterRegistryFacet.sol,
 * ~760-783) отвергает по очереди: «не тот арбитр» → «время взятия неизвестно» →
 * «уже записано» → «рано». Однократность стоит РАНЬШЕ пола намеренно: якорь пола
 * переставляется при каждом взятии спора, а запись о молчании — нет, поэтому
 * арбитр, уже сделавший запись и перевзявший спор, упирается сперва в пол. Скажи
 * интерфейс в этом месте «можно будет через сутки» — он пообещает то, чего не
 * будет: через сутки цепь ответит `NoResponseAlreadyRecorded`. Ровно эту ложь
 * Задача 2 убрала из контракта, и возвращать её в интерфейс нельзя.
 *
