...
async function createStand(): Promise<ChatStand> {
  ...
  return {
    ...
    async stop() {
      ...
      // Гейт «один стенд на процесс» освобождается ТОЛЬКО здесь, после
      // полной остановки — а не в начале stop(): следующий startChatStand()
      // не должен получить право стартовать, пока сервер/склад этого стенда
      // ещё физически не убраны (иначе то же окно гонки, которое гейт и
      // призван закрыть, просто сдвигается на пару строк).
      activeStand = false;
    },
  };
}
