...
describe('адресный бюджет чтения общий у вкладок', () => {
  ...
  it('ЗАМЕР: нет хранилища — потолок ВСЁ РАВНО держится, по вкладке', async () => {
    // ⚠️ ЭТОТ ЗАМОК ПОЯВИЛСЯ ПОСЛЕ СОБСТВЕННОГО ПРОМАХА. Первая версия правки
    // считала только через `localStorage`, и на устройстве без него (приватный
    // режим, сторонний контекст с запрещёнными куками) не считала НИЧЕГО:
    // замер К-1 показал 300 скачиваний за один тик вместо ста. То есть правка
    // вернула ровно тот дефект, который до неё закрывал счётчик в хуке.
    //
    // Что красит: снятие слоя «память вкладки». Тогда без хранилища бюджет
    // снова пропускает всё подряд.
    delete (globalThis as { localStorage?: unknown }).localStorage;
    const seen = installRelayer();
    const { one } = await twoTabs();
    ...
    let admitted = 0;
    for (let i = 0; i < 200; i++) {
      try { await one.fetchBag(PASS, `${ADDR}/${i}-d.bin`); admitted++; } catch { /* отказ бюджета */ }
    }
    console.log(
      `[бюджет замер] без хранилища: попыток 200, ушло в сеть ${seen.reads}, сервер отбил ${seen.refused}`,
    );
    // Работать важнее, чем считать: чтения идут.
    expect(admitted).toBeGreaterThan(0);
    // Но потолок одной вкладки держится и без общей памяти.
    expect(seen.reads).toBeLessThanOrEqual(120);
    expect(seen.refused).toBe(0);
  }, 60_000);
});
