...
/** ⚠️ ЛЮБОЕ новое поле обязано быть ФИКСИРОВАННОЙ ширины (как все пять
 *  нынешних: uint256×2, bytes32×2, address — ровно 148 = 32+32+32+20+32
 *  байт на любое звено, заперто тестом на `linkPreimage`, раунд 4).
 *
 ...
 *  ОДНИ И ТЕ ЖЕ байты. Это верно только пока encodePacked кодирует каждое
 *  поле фиксированным числом байт — тогда границы полей в преимидже
 *  однозначны просто по позиции. Поле переменной длины (например `bytes`
 *  подписи отправителя, которую добавит план 3) делает границы плавающими:
 *  два разных набора значений полей могут дать ИДЕНТИЧНУЮ конкатенацию
 *  байт (классический пример: "ab"+"cdef" и "abcd"+"ef" совпадают
 *  побайтно, если длины не зафиксированы). Тогда «отпечаток сошёлся ⟹
 *  то же самое звено» ломается молча — при полностью зелёных тестах,
 *  потому что сам keccak256 продолжает работать идеально, просто ему на
 *  вход пришли неоднозначные байты. Свойство хеша, которое здесь нужно, —
 *  стойкость ко ВТОРОМУ ПРООБРАЗУ (для конкретного известного отпечатка
 *  трудно найти ДРУГОЕ звено с тем же отпечатком), а не коллизионная
 *  стойкость (для произвольной пары) — но она бесполезна, если сама
 *  упаковка неоднозначна ещё до хеширования. */
 ...
 *  Экспортирован отдельно от linkHash ради теста на ширину (148 байт,
 *  раунд 4) — тест обязан измерять РЕАЛЬНЫЙ преимидж, которым пользуется
 *  linkHash, а не копию кодирования, независимо вычисленную в тесте
 *  (копия проверяла бы только то, что тест написан верно, не то, что
 *  делает сама функция). */
...
export function verifyChain(links: ChainLink[], opts?: ChainAnchor): ChainVerdict {
  ...
  // пять полей последнего звена (коллизионная стойкость keccak256), а
  // значит и его prevHash — а тот уже подтверждён связностью C2 как
