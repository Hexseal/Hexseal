...
import { keccak256, concat, stringToBytes, hashTypedData, recoverAddress } from 'viem';
...
export type ChatSessionErrorCode =
  ...
  /** Ключа на устройстве нет, а заводить не просили (`createIfMissing: false`).
   *  НЕ ПОЛОМКА: единственный ответ, который можно дать, не потревожив
   *  кошелёк. Заведение стоит окна подписи, и платить за него человек должен
   *  тогда, когда пришёл в чат, а не когда подключил кошелёк на любой
   *  странице (К-3). */
  | 'session_absent'
...
async function readRecord(key: string): Promise<StoredSession | null> {
  ...
  if (raw.origin === 'recovery' && raw.recoveryCode !== undefined) {
    if (!(await validRecoveryCode(raw.recoveryCode))) {
      ...
      // Отдаём ОПРЯТНЫМ, а не как лежит: проверяли-то мы нормализованный, и
      // показывать человеку другой текст, чем тот, который признан годным, —
      // расхождение на ровном месте (мелочь третьей проверки).
      raw.recoveryCode = normalizeRecoveryCode(raw.recoveryCode);
    }
  }
  return raw;
}
 ...
 *
 *  NFKD — до всего остального, тот же порядок, что внутри BIP-39, и нужен он
 *  РАДИ ПОЛНОШИРИННЫХ БУКВ (восточные раскладки: «ｌｅｇａｌ» → «legal»).
 *  Неразрывный пробел, который назывался тут причиной раньше, ловится
 *  обычной свёрткой пробелов ниже и без всякого NFKD — причина была названа
...
export interface OpenSessionOptions {
  ...
  /**
   * Заводить ли ключ, если на устройстве его нет. Умолчание — `true`:
   * прежнее поведение, и менять его молча нельзя — всякий, кто не передал
   * опцию, иначе тихо перестал бы заводить ключ, и чат не открылся бы вовсе.
   *
   * ⚠️ `false` — ЧТЕНИЕ БЕЗ ПРАВА ЗАВОДИТЬ. Прочитать ключ с устройства
   * бесплатно и молча; завести — это окно подписи в кошельке. Разные по цене
   * действия, и раньше они были одним: хук открывал сеанс, как только
   * появлялся адрес, а живёт он в шапке — то есть на КАЖДОЙ странице
   * (находка К-3). С `false` отсутствие ключа отвечается `session_absent`,
   * и кошелёк не тревожат вовсе.
   */
...
/** Длина обычной ECDSA-подписи (`r ‖ s ‖ v`) в байтах. Ровно столько
 *  принимает `deriveChatKeypair` (chatCrypto.ts) — два места обязаны
 *  сходиться. */
const PLAIN_SIGNATURE_BYTES = 65;

/** Хоть сколько-то похоже на hex-строку с целыми байтами. */
const ANY_SIGNATURE_RE = /^0x([0-9a-f]{2})+$/;

/** EIP-712-дайджест той самой структуры, которую подписывает кошелёк.
 *  Считается один раз: структура фиксирована. */
const CHAT_KEY_DIGEST = hashTypedData(CHAT_KEY_TYPED_DATA);
...
/**
 * Действительно ли эта подпись сделана ключом ЭТОГО адреса.
 *
 * Восстановление адреса из подписи — чистая местная арифметика: ни узла, ни
 * сети, ни ожидания. Именно это и делает признак настоящим, а не подпоркой.
 *
 * `false` на любом отказе восстановления (негодный признак чётности, мусор в
 * `r`/`s`): «доказать не смогли» — значит не обычный кошелёк.
 */
async function signedByAccount(signature: `0x${string}`, address: `0x${string}`): Promise<boolean> {
  try {
    const recovered = await recoverAddress({ hash: CHAT_KEY_DIGEST, signature });
...
/**
 * Заводит личность, спрашивая кошелёк — и род кошелька определяет по тому,
 * **действительно ли эта подпись сделана ключом ЭТОГО адреса**.
 *
 * ─── ТРИ СТУПЕНИ ────────────────────────────────────────────────────────
 *
 *  1. короче 65 байт → `signature_malformed`. Настоящая подпись, проверяемая
 *     контрактом, короче не бывает;
 *  2. ровно 65 байт → восстановить адрес и сверить с адресом аккаунта.
 *     Совпал → обычный кошелёк, ключ выводится из подписи. Не совпал или
 *     восстановление бросило → кошелёк-контракт;
 *  3. длиннее 65 байт → кошелёк-контракт.
 *
 * ─── ПОЧЕМУ НЕ КОД НА ЦЕПИ И НЕ ОДНА ЛИШЬ ДЛИНА ─────────────────────────
 *
 * Признак менялся дважды, и оба прежних ошибались на живых кошельках.
 *
 * **Код на цепи** ошибался на двух родах из четырёх: делегированные EIP-7702
 * («умный аккаунт» MetaMask) — код есть, а кошелёк обычный; **счётные**
 * смарт-кошельки (Coinbase Smart Wallet до первой транзакции) — кода нет, а
 * кошелёк контрактный, и такой человек не имел дороги в чат вообще.
 *
 * **Одна лишь длина** ошибалась на третьем: **Safe с порогом 1** — самая
 * обычная его настройка — отдаёт РОВНО 65 байт, потому что подписывает
 * владелец. Мы звали его обычным и выводили ключ из подписи ВЛАДЕЛЬЦА, а не
 * кошелька. Сменил владельца (ради чего смарт-аккаунт и заводят) или
 * подписал другим владельцем при пороге 1-из-N — другой ключ, вся переписка
 * нечитаема, и кода восстановления ему не положено.
 *
 * Принадлежность — признак прямой: он спрашивает ровно то свойство, на
 * котором стоит вывод ключа. Восстановление адреса из подписи — чистая
 * местная арифметика, ни узла, ни сети.
 *
 * ─── ЦЕНА И ЧЕСТНО НАЗВАННЫЕ ДЕГРАДАЦИИ ─────────────────────────────────
 *
 * Кошелёк-контракт один раз подпишет то, что мы выбросим. Платится однажды.
 *
 * **Делегированный аккаунт, подписывающий СЕАНСОВЫМ ключом** (не ключом
 * самого адреса), уедет в контрактную ветку: восстановленный адрес не
 * сойдётся с адресом аккаунта. Он получит код восстановления вместо
 * выводимого ключа — это деградация, а не потеря: чат работает, запасной
 * выход есть.
 *
 * Признак чётности в виде 0/1 вместо 27/28 — ЗАМЕРЕНО, что `recoverAddress`
 * (viem) его принимает и восстанавливает верно. Такой кошелёк остаётся
 * обычным, деградации нет.
 *
 * **Сжатая подпись 64 байта (EIP-2098) → отказ**, а не догадка: замерено,
 * что из одних и тех же 64 байт получаются ТРИ разных ключа (две развёртки
 * признака чётности плюс сырая), то есть развернуть её однозначно нечем.
 * Громкий отказ лучше постоянного кода восстановления, заведённого молча.
 *
 * ⚠️ **Личность ЛИПКАЯ.** Установленный род ложится на диск и больше не
 * переспрашивается. Значит кошелёк, ведущий себя по-разному на разных
 * устройствах (здесь подписал сам, там — сеансовым ключом), получит ДВЕ
 * РАЗНЫЕ ЛИЧНОСТИ, и переписка разойдётся молча. Сверить их между собой
 * нечем: это делает справочник открытых ключей, то есть слой хука.
 *
 * @throws {ChatSessionError} `signature_malformed` — не строка, не hex,
 *   пусто или короче 65 байт.
 */
async function establishIdentity(
  ...
  // Регистр приводится ДО всего: кошельки отдают hex-цифры по-разному, а
  // `deriveChatKeypair` принимает только нижний. Без приведения подпись
  // верхним регистром означала бы «чата нет вовсе» — ядро считает эту защиту
  // обязательной, и на этом уровне она обязана быть такой же.
  ...
  // Ступень 1: КОРОЧЕ обычной подписи — это мусор, а не «подпись другого
  // вида». Настоящая подпись, проверяемая контрактом, короче 65 байт не
  // бывает. Пол обязателен: без него заглушка провайдера в один байт
  // НЕОБРАТИМО переводила обычного человека на код восстановления — запись
  // ложилась на диск, и починившийся кошелёк уже никто не спрашивал
  // (находка В-1 четвёртой проверки).
  if (bytes < PLAIN_SIGNATURE_BYTES) {
    throw new ChatSessionError(
      `chatSession: подпись короче ${PLAIN_SIGNATURE_BYTES} байт (${bytes}) — это не подпись`,
      'signature_malformed',
    );
  }
 ...
 *
 * ⚠️ Кошелёк-контракт — ТОЖЕ ОДНО окно, а не «ни одного вовсе», как здесь было
 * написано до финальной проверки ветки. Подпись просят ВСЕГДА: она и есть
 * признак рода — восстановленный из неё адрес либо сходится с адресом
 * аккаунта (обычный кошелёк, ключ выводится из подписи), либо не сходится
 * (кошелёк-контракт, ключ случайный плюс код восстановления). Родом занимается
 * сама подпись, а не отдельный вопрос к цепи, — это описано на 115 строк выше
 * в шапке файла, и там сказано верно: «кошелёк-контракт один раз подпишет то,
 * что мы выбросим. Платится однажды». Две редакции одного факта разошлись, и
 * ближняя к функции была ложной.
 */
...
async function doOpenSession(
  ...
  // Заводить не просили — уходим ДО замка и ДО КОШЕЛЬКА, обеими ветками.
  //
  // ⚠️ ОТКАЗ ЧТЕНИЯ ТОЖЕ УХОДИТ ЗДЕСЬ, и это не небрежность. Ниже, в
  // `openWithoutStorage`, подпись спрашивается РАДИ ОДНОГО — узнать род
  // кошелька, чтобы решить, можно ли жить на выведенном ключе в памяти
  // вкладки. Но раз заводить нам не разрешили, действовать по этому ответу
  // всё равно нечем: окно подписи всплыло бы и ничего не изменило. Ровно та
  // цена, ради избавления от которой заведена опция (К-3).
  //
  // Коды при этом РАЗНЫЕ: «там пусто» и «не смогли прочитать» — разные
  // вещи, и для кошелька-контракта разница стоит его личности (К-4).
  // Показывает их `lib/chatSessionOff.ts`, каждый своими словами.
  if (opts.createIfMissing === false) {
    if (first.status === 'empty') {
      throw new ChatSessionError(
        'chatSession: ключа чата на этом устройстве нет, а заводить не просили',
        'session_absent',
      );
    }
  ...
  return withCrossTabLock(`hexseal-chat-session-${key}`, timeoutMs, async () => {
    ...
    const identity = await establishIdentity(address, signTypedData);
...
async function restoreFromCodeUnderLock(
  ...
  // на этом устройстве.
  const { kind } = await establishIdentity(address, signTypedData);
  if (kind !== 'contract') {
    throw new ChatSessionError(
...
export function exportRecoveryCode(session: ChatSession): string {
  // Несущая половина здесь — `walletKind`: она ловит запись, у которой
  // подменено происхождение (заперто тестом «род лежит В ЗАПИСИ и сверяется
  // при выдаче кода»). Половина про `origin` — защита в глубину: входа,
  // достижимого сегодня, у неё нет, снятие её ни одного теста не красит.
  // Сказано прямо, потому что прежняя формулировка («два независимых
  // условия, а не одно с запасом») замером не подтверждалась.
 ...
 */
export interface ForgetSessionOptions {
  /**
   * Человек подтвердил снятие, ЗНАЯ цену. Без этого не снимается ничего.
   *
   * ⚠️ ЭТО ОХРАНА, А НЕ ЦЕРЕМОНИЯ, и стоит она здесь по замеру. Прежняя
   * охрана жила в интерфейсе — подтверждающее окно, разное по роду кошелька
   * (К-1). Сквозная проверка обошла её одной строкой: `onClick={() =>
   * disableChat()}` снимал ключ одним нажатием, и из 1338 тестов не краснел
   * ни один, потому что замок проверял НАПИСАНИЕ кнопки.
   *
   * Здесь обойти нельзя иначе как вслух: вызывающий, ничего не сказавший,
   * не стирает ничего. Отказ ЗАКРЫТЫЙ — забывчивость не стоит человеку его
   * переписки, а для кошелька-контракта это была бы переписка НАВСЕГДА.
   */
  acknowledged?: boolean;
}

/**
 * Снимает ключ чата с устройства.
 *
 * @returns `true` — сняли (или снимать было нечего); `false` — не сняли,
 *   потому что подтверждения не было. Возврат существует, чтобы вызывающий
 *   мог отличить «сделано» от «отказано», а не считать молчание согласием.
 *
 * @throws {ChatSessionError} `forget_failed` — удалить не удалось. Молча
 *   вернуть «забыто» значило бы соврать про то, что ключ всё ещё на диске.
 */
export async function forgetSession(
  address: `0x${string}`,
  opts: ForgetSessionOptions = {},
): Promise<boolean> {
  const key = storageKey(address); // форма адреса — до всего остального
  // Проверка подтверждения — ПОСЛЕ разбора адреса и ДО любого касания диска.
  if (!opts.acknowledged) return false;
  if (!idbFactory()) return true; // хранилища нет — забывать нечего
  try {
    await idbDelete(key);
    return true;
  } catch (err) {
