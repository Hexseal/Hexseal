 ...
 * четырёх шагов (решение владельца, 21 августа 2026):
...
/* ────────── пара чисел моложе арбитра ────────── */

/**
 * Надо ли объяснить читателю, почему серия ошибок длиннее пары чисел.
 *
 * ⚠️ ЭТО НЕ ОШИБКА ОТОБРАЖЕНИЯ И НЕ ОШИБКА ЦЕПИ. Владелец увидел на
 * живом экране «`judged` 0, `overturned` 0» и строкой ниже «ошибок подряд:
 * 1 из 3» — и прочёл это как противоречие. Цепь при этом сказала ровно то,
 * что у неё записано (сверено чтением 22 августа 2026).
 *
 * Пара уже серии СРАЗУ ПО ДВУМ ГРАНИЦАМ, и обе видны в коде контракта:
 *
 * 1. **по времени** — `overturnedVerdicts` заведён разрезом и начал с нуля;
 *    задним числом он не наполняется. `arbiterMistakeStreak` лежал в
 *    хранилище и до разреза, то есть несёт историю целиком;
 * 2. **по роду** — `_recordArbiterMistake` поднимает серию ВСЕГДА, а
 *    `overturnedVerdicts++` делает только на `OwnerOverturn` и `AppealVote`.
 *    Просрочка в пару не входит НИКОГДА, и это намеренно.
 *
 * ⚠️ ИМЕННО ПОЭТОМУ НАДПИСЬ НЕ НАЗЫВАЕТ ПРИЧИНУ. Из чего сложилась
 * конкретная единица, цепь НЕ ХРАНИТ — она хранит счётчик, а не историю.
 * «Это была просрочка» — вывод исключением (в ленте нет ни одного
 * `VerdictOverturned`), верный сегодня и ломающийся завтра, как только
 * случится первый настоящий переворот. Надпись говорит про ГРАНИЦУ
 * счётчиков, а не про событие.
 *
 * ⚠️ И ПОЭТОМУ ЖЕ ОНА НЕ ПОСТОЯННАЯ. Объяснительная строка на КАЖДОЙ
 * карточке превращается в шум, который перестают читать, и тогда её не
 * прочтут там, где она нужна.
 *
 * Отдельной ветки на «серия ноль» здесь нет и не нужно: при
 * `mistakeStreak === 0` условие `0 < 0` ложно само по себе.
 *
 * @param mistakeStreak      ошибок подряд сейчас (`arbiterMistakeStreak`).
 * @param overturnedVerdicts сколько вердиктов перевернули за всю службу.
 * @returns `true`, если пара чисел НЕ может объяснить всю серию.
 */
export function streakOutrunsPair(
  mistakeStreak: number,
  overturnedVerdicts: number,
): boolean {
  return overturnedVerdicts < mistakeStreak;
}
