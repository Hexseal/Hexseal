...
export interface ReadAnchorsOptions {
  ...
  /**
   * Спрашивать ли ЛЕНТУ. Умолчание — `true`.
   *
   * ⚠️ ЭТО НЕ НАСТРОЙКА «БЫСТРЕЕ/МЕДЛЕННЕЕ», А РАЗНЫЕ ВОПРОСЫ У ДВУХ ЧИТАТЕЛЕЙ,
   * и стоит эта разница двенадцать `eth_getLogs` на каждое открытие чата:
   *   — АРБИТР спрашивает «в каком ПОРЯДКЕ»: отпечаток на блоке N, запись о
   *     молчании на блоке M. Номеров блока не отдаёт ни один геттер, только
   *     лента, и без неё весь смысл затеи пропадает — `withLog: true`;
   *   — СТОРОНА спрашивает «отмечено ли МОЁ»: это сравнение её отпечатка со
   *     списком геттера, один дешёвый `eth_call`. Номер транзакции она не
   *     рисует нигде (`PresentAnchorLine` знает ровно два слова — «отмечено» и
   *     «не отмечено»), блоков не показывает, порядка не строит.
   *
   * ⚠️ ЧЕСТНОЕ РАЗЛИЧЕНИЕ ОТ ЭТОГО НЕ СТРАДАЕТ, и это проверено, а не
   * предположено. «Не отмечено» и «не знаем» (Задача 7) считаются по ГЕТТЕРУ и
   * только по нему: `unread` — это либо молчание геттера, либо неполный список
   * (`digestsComplete === false`), см. `anchorBy`. Лента на вердикт не влияет
   * ни в одной ветке — она добавляет номер блока, и всё.
   *
   * ⚠️ ЧЕГО НЕЛЬЗЯ ДЕЛАТЬ С `false`: строить по такому ответу ПОРЯДОК. Поля
   * ленты честно пусты, `logsComplete === false`, `window === null` — то есть
   * `anchorOrder` скажет `out_of_window`, «громкую потерю», а не выдумает
   * `no_record`. Молчаливого вранья здесь нет, но и звать `anchorOrder` на
   * такой ответ незачем.
   */
  withLog?: boolean;
}
 ...
 *
 * ⚠️ ЛЕНТА НЕ СПРАШИВАЕТСЯ ВОВСЕ В ДВУХ СЛУЧАЯХ, И ОБА ПРО ЦЕНУ. Первый —
 * отпечатков по сделке нет: двенадцать запросов `eth_getLogs` ради пустого
 * ответа платил бы каждый открывший ящик по обычной сделке. Второй —
 * `withLog: false`: спросивший заранее знает, что порядок ему не нужен (см.
 * поле в `ReadAnchorsOptions`); так читает СТОРОНА.
 */
export async function readChainAnchors(
  ...
  // ⚠️ ЛЕНТУ СПРОСИЛИ НЕ ВСЕ, КТО СПРАШИВАЕТ ПРО ОТПЕЧАТКИ. Стороне нужен
  // ВЕРДИКТ («отмечено ли моё»), а он весь в геттере; номеров блока она не
  // рисует нигде. Двенадцать `eth_getLogs` на каждое открытие чата по спорной
  // сделке не покупали ей ничего. Поля ленты при этом остаются ЧЕСТНО пустыми
  // (`logsComplete: false`, `window: null`) — «не знаем», а не «нет».
  if (opts.withLog === false) return base;
...
export function anchorFromChain(digest: Hex, anchors: ChainAnchors | null): AnchorState {
  ...
  if (a.verdict === 'match') {
    ...
    // ⚠️ НА БОЕВОМ ПУТИ СТОРОНЫ ОН ВСЕГДА `null`, и это не поломка: она читает
    // с `withLog: false`, потому что не рисует его нигде (`PresentAnchorLine`).
    // Поле оставлено — им пользуется арбитр и им же закрыт H2.
