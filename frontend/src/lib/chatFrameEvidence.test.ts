...
// проверяльщик не греет НАПРЯМУЮ, и это главное доказательство испр. 4: будь
// «сначала прогрей» ещё условием работы, T1-T11 (тесты, что не зовут
// `receiveBags`) покраснели бы — и краснеют.
//
// ⚠️ ИЗМЕРЕННОЕ число — 12 и 11 (не 13 и 12, как в исходном плане): зелёными
// остаются ТРИ теста, а не два — T12 (греет проверяльщик ПОБОЧНО, через
// `receiveBags` → `readyFrameVerifier`), T13 (кадры не судит вовсе) и,
// неожиданно, T14. T14 краснеть ОБЯЗАН по плану (он тоже зовёт `judge`, а
// прогрева не делает), но идёт по файлу ПОСЛЕ T12 — а module-уровневое
// состояние обеих мутаций-возвратов к v1 живёт МЕЖДУ тестами внутри одного
// файла (vitest изолирует по ФАЙЛУ, не по тесту), и `_resetFrameVerifierForTest`
// в `beforeEach` его не трогает (сбрасывает только то состояние, что знает
// сегодняшний код). Проверено ПРЯМЫМ измерением, не рассуждением: T14 в
// изоляции (`npm test -- src/lib/chatFrameEvidence.test.ts -t T14`, ни один
// другой тест не запускается) под теми же мутациями КРАСНЕЕТ; связка «только
// T12, затем T14» (`-t "T12|T14"`) — ЗЕЛЕНЕЕТ. Порядок объявления реально имеет
// значение для ЭТОГО теста под ЭТОЙ мутацией — числа и команды в
// `task-4-report.md`, раздел «Задача 4в-1» (там же — почему это НЕ то же самое,
// что заниженный `coldMs` у T14 на обычном, немутированном коде: тот эффект —
// самопрогрев `libsodium-wrappers` внутри собственного цикла T14, не зависит
// от T12 вовсе, и живёт отдельным замером в `chatFrameEvidenceColdStart.test.ts`).
...
describe('цена проверки', () => {
  it('T14: ЗАМЕР — тысяча кадров судится за разумное время, все ok', async () => {
    ...
    // Первый кадр судится ОТДЕЛЬНО от остальных 999 — но НЕ ради цены холодной
    // загрузки библиотеки: то число здесь заведомо ЛОЖНОЕ, и не измеряется, а
    // печатается только для любопытства (без `expect`, намеренно).
    //
    // ⚠️ ПОЧЕМУ ЛОЖНОЕ. `_resetFrameVerifierForTest()` в `beforeEach` сбрасывает
    // только НАШЕ обещание готовности — а не сам модуль `libsodium-wrappers` и
    // не его собственное состояние `ready`. Цикл ВЫШЕ уже вызвал `honestFrame()`
    // 1000 раз, и КАЖДЫЙ раз она САМА делает `await import('libsodium-wrappers')`
    // + `await sodium.ready` (для подписи звена) — тем же самым модулем, что
    // потом читает `frameVerifier()`. Раз модуль уже загружен и `ready`
    // разрешено, второй `import` того же спецификатора и второй `await` уже
    // разрешённого промиса — почти бесплатны. Проверено замером: `firstMs`
    // печатает `0-1 мс` даже когда этот тест запущен В ИЗОЛЯЦИИ (`npm test --
    // src/lib/chatFrameEvidence.test.ts -t T14`, без единого другого теста в
    // файле) — то есть дело не в соседних тестах, а в собственном цикле выше.
    // Настоящая цена холодного вызова (там, где ДО таймера `libsodium-wrappers`
    // никто не трогал вовсе) — отдельный файл
    // `chatFrameEvidenceColdStart.test.ts`: около 13-14 мс, с потолком-замком,
    // который проверено умеет краснеть (искусственная задержка ~2.5 с в
    // `frameVerifier()` даёт красный на реальном числе — см. докстринг там же и
    // `task-4-report.md`).
    ...
    console.info(`[замер, без утверждения — см. докстринг] verifyFrameEvidence: первый кадр `
      + `(УЖЕ ТЁПЛЫЙ, не холодный модуль) ${firstMs} мс, остальные 999: ${elapsed} мс`);
    expect(ok).toBe(1000);
