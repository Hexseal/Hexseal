...
describe('потолок размера файла: одно число, и оно верное', () => {
  ...
  it('ОБА места берут число из константы, и ни одно не вписывает его руками', () => {
    // Красит: `{ mb: 200 }` в ЛЮБОМ из двух мест. Тогда снижение потолка
    // снова разошлось бы с тем, что видит человек, — ровно как случилось с
    // «5 ГБ».
    //
    // ⚠️ СЧИТАЮТСЯ ОБА, А НЕ «ХОТЬ ОДНО». Первая версия искала одно
    // вхождение — и осталась зелёной, когда мутация вписала число руками в
    // отказ по размеру: регулярка нашла ВТОРОЕ место, подпись к скрепке
    // (мутация М-41 прошла незамеченной).
    const panel = read('components/ChatPanel.tsx');
    const fromConstant = panel.match(/mb:\s*MAX_FILE_SIZE\s*\/\s*\(1024\s*\*\s*1024\)/g) ?? [];
    expect(fromConstant).toHaveLength(2);
    expect(panel).not.toMatch(/\bmb:\s*\d/);
  });
