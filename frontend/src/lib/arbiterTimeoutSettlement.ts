 ...
 *
 *  • `arbiter == 0` — за спор никто не взялся: исход внутри этой ветки решает
 *    явка (`clientResponded`/`executorResponded`). Откликнулись обе стороны —
 *    котёл делится, floor(pot/2) исполнителю, остаток клиенту
 *    (`DisputeSplitNoVerdict`); молчала одна — ей четверть котла, остаток
 *    явившемуся (`DisputeUnanswered`);
 ...
 *
 * И четвёртый исход, 'unanswered', — молчание одной из сторон внутри ветки
 * `arbiter == 0` (см. пункт выше). Отдельный вид union, а не флаг внутри
 * 'split': слова другие — «никто не рассудил, значит никто не выиграл» для
 * стороны, которая недополучила ЗА молчание, было бы неправдой.
 */
