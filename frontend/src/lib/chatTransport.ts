 ...
 *
 * НЕ ЗНАЕТ: про модуль шифрования содержимого, про модуль, что строит
 * цепочку сообщений, про React, про кошелёк напрямую — `signMessage`
 * приходит аргументом, не импортом. Единственная связь с подписью —
 * вызывающая сторона передаёт функцию, умеющую подписать строку; что она
 * делает внутри (viem, ethers, встроенный кошелёк), этому модулю не важно
 * и не видно. Проверяется буквально — импортов этих двух модулей и React
 * в файле нет вовсе (`script/` или ревью прогоняют grep по этому файлу).
 *
 ...
 * Если пропуск не обновляется (сервер лежит, часы на устройстве сильно
  ...
  const loop = async () => {
    ...
    while (!stopped) {
      ...
      try {
        ...
        if (stopped) return;
        // Долбят нарочно (429): отступаем минимум на Retry-After сервера, а
        // не бьём с базовым интервалом в ту же стену лимитера.
        if (err instanceof BagRateLimitError) {
          waitMs = Math.max(waitMs, err.retryAfterSec * 1000);
        }
        opts.onError?.(err);
      }
      if (stopped) return;
