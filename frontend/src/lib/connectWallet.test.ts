...
describe('точка запуска подключения одна', () => {
  ...
  it('обе кнопки подключения ходят через хук', () => {
    ...
    expect(callers).toEqual([
      ...
      // ⚠️ ТРЕТИЙ ЗВАВШИЙ ДОБАВЛЕН ОСОЗНАННО, как и требует этот замок.
      // «Переподключить кошелёк» (журнал живого телефона: протухший сеанс
      // WalletConnect, подпись доставить некому) обязано подключать ТЕМ ЖЕ
      // путём, что обе кнопки: своё подключение здесь означало бы второй
      // маршрут, который разъедется с первым молча. Разбор — шапки
      // `hooks/useWalletReach.ts` и `lib/walletReach.ts`.
      'hooks/useWalletReach.ts',
    ]);
  });
