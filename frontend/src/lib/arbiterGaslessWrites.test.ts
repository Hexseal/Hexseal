...
describe('ответ снятого арбитра идёт через релеер', () => {
  ...
  it('R11 ОТВЕЧАЕТ ТОТ, КТО УЖЕ НЕ АРБИТР: путь не спрашивает роль ни у кого', async () => {
    // Главное обстоятельство этой обёртки. Снос снимает статус — значит любая
    // проверка «а он арбитр?» по дороге закрыла бы дверь ровно перед тем, для
    // кого она заведена. Замеряется не текстом, а составом обращений к цепи:
    // единственное законное чтение на пути через релеер — счётчик форвардера.
  ...
  it('R12 relay down — прямая транзакция ТОЙ ЖЕ калдатой и с потолком таблицы', async () => {
    ...
    expect(CEILINGS.respondToRemoval).toBe(80_000n);
  });
