 ...
 * применить его (`applyKey` — заявка на спор либо публикация ключа отдельной
 * транзакцией).
 *
 * ⚠️ Второй параметр зовётся `applyKey`, а НЕ `useKey`, и это не вкусовщина:
 * `eslint-plugin-react-hooks` разбирает имена, а не смысл, и на `useKey(keys)`
 * внутри обычной функции выдавал `rules-of-hooks` — то есть гейт хуков в CI был
 * КРАСНЫМ на чистом дереве, и любой чужой PR приезжал бы «сломанным» ещё до
 * первой своей строки. Переименование лечит это, не выключая правило.
 *
 ...
 * `deriveKey`/`applyKey` и настоящим `chatSignatureGate.ts`.
 *
 ...
 *   3. `requireSignatureGate(false)` — перед `applyKey()`. Честно наоборот:
