 ...
 *
 * ⚠️ ЧЕСТНО О ГРАНИЦЕ ЗАЩИТЫ. Из подписного ключа собеседника вывести ничего
 * нельзя из его же ключа ШИФРОВАНИЯ: подписной выводится из ЗАКРЫТОЙ половины,
 * а нам доступна только открытая. Значит ключ обязан прийти извне.
 *
 * Источник у него есть и он готов: справочник Задачи 2 (`relayer/directory.js`,
 * `GET /keys/:address`) хранит рядом с `boxKey` ещё и `signKey` — 32 байта hex,
 * ровно ширина открытой половины Ed25519, которую этот модуль и производит.
 * Поле сегодня необязательное и никем не заполняется: публикует его тот, кто
 * заводит сеанс, то есть хук Задачи 6, а не этот модуль.
 *
 * Пока `peerSigningPublicKeys` не передан, проверка подписи — это проверка
 * САМОСОГЛАСОВАННОСТИ: она ловит подмену одного звена, порчу байтов по дороге и
 * смену ключа посреди переписки (ключ прибивается к самому раннему звену), но
 * НЕ ловит того, кто перепишет всю цепочку целиком своим ключом. Полное
 * связывание «ключ чата ↔ адрес в цепи» §11 общей спеки оставляет следующим
 * планам — здесь готовы обе половины стыка, кроме самой проводки.
 *
