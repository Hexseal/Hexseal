 ...
 *
 * ⚠️ ЧЕСТНО О ГРАНИЦЕ ЗАЩИТЫ. Подписной ключ собеседника взять СЕГОДНЯ
 * неоткуда: справочник (`relayer/directory.js`, Задача 2) хранит один
 * 32-байтный ключ — ШИФРОВАНИЯ, — а вывести из него подписной невозможно (он
 * выводится из ЗАКРЫТОЙ половины). Поэтому без `peerSigningPublicKeys`
 * проверка подписи — это проверка САМОСОГЛАСОВАННОСТИ: она ловит подмену
 * одного звена, порчу байтов по дороге и смену ключа посреди переписки, но
 * НЕ ловит того, кто перепишет всю цепочку целиком своим ключом. Связывание
 * «ключ чата ↔ адрес в цепи» §11 общей спеки прямо оставляет следующим
 * планам; здесь для него готово место — и не более того.
 *
 ...
 * ящике, и вердикт у каждой свой (`chains`). Мешок постороннего не может
 * испортить вердикт собеседнику.
 *
...
/** Голова в памяти вкладки — запасной путь, когда хранилища нет вовсе.
 *  Ровно то, что обещает предупреждение выше: одна вкладка, до перезагрузки. */
...
export async function sendMessage(
  ...
  return withCrossTabLock(`hexseal-chat-conv-${own}-${peerAddr}`, lockTimeoutMs, async () => {
    ...
    const stored = idbFactory() ? await readHeadRecord(id) : (_memoryHeads.get(id) ?? null);
    ...
    if (persisted) persisted = await writeHeadRecord(id, confirmed);
    if (!persisted) _memoryHeads.set(id, confirmed);

    return { link, signature, signerPublicKey: signer.publicKey, frame, key, payload, persisted };
  });
}
...
export async function receiveBags(
  ...
  if (opts.own && opts.own.length > 0) {
    ...
    for (const s of opts.own) {
      if (onlyPeer && s.link.sender !== own) continue;
      messages.push({
