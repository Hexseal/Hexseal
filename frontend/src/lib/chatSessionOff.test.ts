...
describe('экран «чат не открылся» разведён по причинам', () => {
  ...
  it('⚠️ «ключа нет, заводить не просили» — это «включите», а не «что-то помешало»', () => {
    // Найдено при написании отчёта, уже после К-3. `session_absent` — не
    // поломка, а самое обычное состояние: человек либо ещё не заходил в чат,
    // либо однажды отказался подписывать. Общий экран сказал бы ему «что-то
    // помешало открыть переписку» — то есть ровно то обвинение, ради
    // избавления от которого делалась К-2, только другими словами.
    //
    // Нажатие «включить» здесь и есть просьба завести ключ: `retry()` снимает
    // прежний отказ и взводит чат.
    const screen = offScreenFor('session_absent');
    expect(screen.titleKey).toBe('chat.messaging_off');
    expect(screen.hintKey).toBe('chat.messaging_off_hint');
    expect(screen.action).toBe('retry');
  });
