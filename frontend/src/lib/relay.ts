...
const GAS_DEFAULTS: Record<string, bigint> = {
  ...
  // Ответ снятого арбитра: один холодный SSTORE и событие. USDC не трогает
  ...
  // max 55 235 / 50 305; 55 235 × 1.19 × 1.2 ≈ 78 876.
  respondToRemoval:       80_000n,
...
/**
 * Ответ снятого арбитра на обвинение — gasless.
 *
 ...
 *
 * ⚠️ ОТВЕЧАЕТ ЧЕЛОВЕК, КОТОРЫЙ УЖЕ НЕ АРБИТР — и весь путь обязан это выдержать.
 * Проверено по каждому звену, а не предположено:
 *   • контракт пускает по `removedAt[caller] != 0`, статуса арбитра не
 *     спрашивает нигде (`isArbiter` в теле функции не читается вовсе) — а
 *     снос как раз этот статус и снимает, так что гейт по статусу означал бы
 *     «ответить может только тот, кого не снимали»;
