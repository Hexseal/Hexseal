...
const GAS_DEFAULTS: Record<string, bigint> = {
  ...
  //
  // ⚠️ ПОДНЯТ 23 августа 2026 вместе с газ-капами в `src/Agreement.sol`
  // (docs/audits/2026-08-22-diamond-death-escrow.md, раздел «Газ»). Перед
  // `autoAwardXP` контракт теперь ТРЕБУЕТ, чтобы `gasleft()` хватало на полную
  // передачу капа 500 000 — иначе ревертит целиком, вместо того чтобы молча
  // оставить исполнителя без XP. Это поднимает не расход, а МИНИМАЛЬНЫЙ ЛИМИТ:
  // цепь по-прежнему берёт по факту (замер расхода не изменился), но лимит ниже
  // порога отдаёт `NotEnoughGasForDiamondCall`.
  //   замер минимального работающего лимита, здоровый даймонд: 703 138
  //   он же при даймонде, который жжёт газ вместо ответа: 1 134 325
  // По формуле файла: 703 138 × 1.19 (настоящий USDC дороже мока) × 1.2 (запас)
  // ≈ 1 004 081, вверх до круглого — заодно накрывает и второй случай.
  ...
  //
  // ⚠️ ПОДНЯТ ВТОРИЧНО 20 августа 2026, С 90 000 ДО 110 000 — вместе с долгом
  // ABI, ОДНОЙ работой (docs/OPEN-ITEMS.md, пункт 91, закрыт ею же). До 20
  // августа `config/contracts.ts` знал `respondToRemoval(bytes32)` без
  // `string`, послать слова было нечем, и 90 000 хватало. Починка ABI в
  // одиночку оживила бы кнопку «возразить словами» и в тот же день упёрла бы
  // её в потолок: человек, у которого только что сожгли залог, получил бы
  // «out of gas» вместо возражения.
  //
  // База — та же ХУДШАЯ ИЗ ДВУХ таблиц полного `forge test --gas-report`, но
  // на сцене с ПОЛНЫМИ 512 байтами и ХОЛОДНЫМИ слотами (`vm.cool`): в цепи
  // каждая транзакция начинается с холодного хранилища, а тёплая цифра выходит
  // только потому, что стенд успел потрогать те же слоты подготовкой.
  //   max 72 174 (прокси) / 56 704 (фасет);
  //   72 174 × 1.19 × 1.2 ≈ 103 064 → вверх до круглого 110 000.
  // Та же сцена без слов даёт прежние 61 661 — то есть база 90 000 занижена не
  // была, ей просто не показывали строку.
  //
  // ⚠️ ПОТОЛОК ОБЯЗАН ПОКРЫВАТЬ ТРАНЗАКЦИЮ ЦЕЛИКОМ, а не исполнение:
  //   72 174 (исполнение) + 21 000 (внутренние) + ≈ 9 060 (калдата 612 байт)
  //   ≈ 102 234. Отсюда же видно, почему 100 000 было бы мало.
  //
  // ⚠️ ДЛИНА СТРОКИ ПОТОЛКУ НЕ ПЕРЕДАЁТСЯ И ПЕРЕДАВАТЬСЯ НЕ БУДЕТ: потолок один
  // на все вызовы, и посчитан он по САМОМУ ДЛИННОМУ законному ответу. Длиннее
  // 512 байт цепь не примет вовсе (`ReasonTooLong`), так что худший случай
  // здесь замкнут, а не оценён.
  //
  ...
  // ⚠️ 200 000 БЫЛО ВРАНЬЁМ ЗАДОЛГО ДО ЭТОЙ ПРАВКИ, и это не регрессия от неё:
  // `resolveDispute` идёт через тот же `_complete`, что и `release`, то есть на
  // первой сделке пары начисляет XP обеим сторонам в холодные слоты. Замер
  // здорового пути — 462 016 против отсечки 200 000, то есть запасной путь
  // («узел не смог оценить») ронял арбитрскую кнопку «вернуть клиенту» /
  // «заплатить исполнителю» в out of gas всегда. Найдено при замере газ-капов
  // 23 августа 2026.
  //
  // Отсечка выравнена с `release` — это буквально тот же хвост функции плюс
  // сбор за спор перед выплатой.
  resolveDispute:       900_000n,
  ...
  // цепь по-прежнему берёт по факту (замер расхода не изменился), но лимит ниже
  // порога отдаёт `NotEnoughGasForDiamondCall`.
  //   замер минимального работающего лимита, здоровый даймонд: 600 980
  //   он же при даймонде, который жжёт газ вместо ответа: 712 018
  // По формуле файла: 600 980 × 1.19 (настоящий USDC дороже мока) × 1.2 (запас)
  // ≈ 858 200, вверх до круглого — заодно накрывает и второй случай.
  release:            900_000n,
  ...
  // цепь по-прежнему берёт по факту (замер расхода не изменился), но лимит ниже
  // порога отдаёт `NotEnoughGasForDiamondCall`.
  //   замер минимального работающего лимита, здоровый даймонд: 601 125
  //   он же при даймонде, который жжёт газ вместо ответа: 712 018
  // По формуле файла: 601 125 × 1.19 (настоящий USDC дороже мока) × 1.2 (запас)
  // ≈ 858 406, вверх до круглого — заодно накрывает и второй случай.
  triggerAutoApprove: 900_000n,
 ...
 *
 * ⚠️ `reply` — ВТОРОЙ АРГУМЕНТ, ОБЯЗАТЕЛЬНЫЙ ЗДЕСЬ И НЕОБЯЗАТЕЛЬНЫЙ В ЦЕПИ.
 * Контракт пустую строку принимает (ответ — право, а не обязанность) и события
 * `RemovalReplyGiven` тогда не порождает. А вот у ЭТОЙ обёртки значения по
 * умолчанию нет намеренно: с умолчанием будущий экран мог бы забыть довезти
 * слова, и человек отправил бы голый отпечаток, будучи уверен, что возразил.
 * Молчать — это отдельное решение, и его придётся написать словом `''`.
 *
 * ⚠️ ДЛИНА СЧИТАЕТСЯ В БАЙТАХ, А НЕ В СИМВОЛАХ, и потолок (512) обязан
 * спрашиваться у цепи — `getMaxReasonBytes()`. Здесь он НЕ проверяется
 * специально: своя копия правила разошлась бы с цепью молча. Форма, которую
 * заведут поверх этой обёртки, обязана показывать остаток по цепному числу;
 * перебор цепь отвергает сама (`ReasonTooLong`).
 */
