...
// признака). Разница только в словах: молчавшему сказано, что дележ вышел
// неровным ИМЕННО ПОТОМУ, что он не ответил; откликнувшемуся — что не ответила
// вторая сторона.
//
// И НИ ОДНОМУ не говорится, в чью сторону перекос. Первая редакция говорила
// «меньше/больше половины», и это ложь того же класса, что и враньё про суммы,
// только про их соотношение: в ветке чёрного списка USDC недоставленная доля
// явившегося уходит клиенту, поэтому молчавший клиент может получить ВЕСЬ
// котёл, а откликнувшийся исполнитель ноль. Данных, чтобы отличить эту ветку,
// на расчётном пути нет по построению — значит и сравнения быть не должно.
describe('refundNotifCopy — дележ без долей, но с признаком, кто молчал', () => {
  ...
  it('молчавшему говорит, что дележ вышел неровным именно поэтому', () => {
    const { title, body } = refundNotifCopy(EXECUTOR_SILENT, 'executor');
    expect(title).toBe('Escrow Split');
    expect(body).toMatch(/didn't answer/i);
    expect(body).toMatch(/not split evenly/i);
    expect(body).toContain('200.00 USDC in total');
  });

  it('откликнувшемуся говорит, что молчала вторая сторона и дележ поэтому неровный', () => {
  ...
  // Клетка на ту самую остаточную ложь: перекос не называется ни в чью сторону
  // ни одной из сторон. Слова «больше/меньше половины» здесь запрещены — они
  // переворачиваются в блэклист-ветке, а отличить её на расчётном пути нечем.
  it('в чью сторону перекос — не говорит НИ ОДНОЙ стороне', () => {
    for (const role of ['client', 'executor'] as const) {
      const { body } = refundNotifCopy(EXECUTOR_SILENT, role);
      expect(body).not.toMatch(/than half/i);
      expect(body).not.toMatch(/bigger|smaller|more than|less than/i);
    }
  });
  ...
  // Точную сумму по-прежнему не называет ни одной стороне — только причину
  // неровности и общий котёл. За цифрой — в кошелёк.
  it('точную долю не называет ни одной стороне, только котёл и причину неровности', () => {
    for (const role of ['client', 'executor'] as const) {
      const { body } = refundNotifCopy(EXECUTOR_SILENT, role);
      expect(body).not.toMatch(/USDC to (you|the client|the executor)\b/i);
      expect(body).toContain('check your wallet');
