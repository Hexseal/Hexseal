...
describe('опрашивать ли ящик — то есть брать ли пропуск', () => {
  ...
  it('записи не было НИКОГДА — НЕТ: мешков для нас быть не может', () => {
    expect(mailboxWorthPolling('absent')).toBe(false);
  });

  it('в справочнике ЧУЖОЙ ключ — ДА, и это исправление', () => {
    // ⚠️ ЗДЕСЬ БЫЛО НАПИСАНО `false`, и это была вторая потеря данных (найдена
    // ревью координатора). Разница между «записи не было никогда» и «запись есть,
    // но ключ другого устройства» решающая: во втором случае на складе могут
    // лежать мешки, запечатанные на наш ПРЕЖНИЙ ключ, и здесь они открываются.
    // Блокируя опрос, мы прятали читаемую переписку.
    expect(mailboxWorthPolling('other_key')).toBe(true);
  });
