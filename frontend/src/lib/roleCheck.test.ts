...
describe('роль читают через общее правило, а не через `!!`', () => {
  it('места, раздающие права по роли, ходят через lib/roleCheck', () => {
    ...
    expect(readers).toEqual([
      ...
      // Читает статус ЧУЖОГО арбитра (того, кто ведёт спор), и прав по нему не
      // раздаёт: `registered` — приписка к снимку, а не отказ. Про сбой решено
      // осознанно и врозь: отказ чтения даёт `registered: null`, и это НЕ то же,
      // что `false` («мы проверили, он снят»). Замеряется мутацией 3 задачи 5 —
      // подмена отдельного чтения выводом из наличия ключа даёт 4 красных.
