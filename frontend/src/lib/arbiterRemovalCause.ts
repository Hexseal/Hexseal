 ...
 * арбитра (`getArbiterStanding`, поле 14 — последнее; было 13, пока пункт 101
 * не вставил `overturnedVerdicts` рядом с `cleanVerdicts`).
 *
 ...
 * `arbiterMistakeStreak`. Проверен факт СЕРИИ, а выбор между двумя именами —
 * заявление обвинителя. То есть `verifiedByChain: true` здесь означает
 * «признак, на который ссылается повод, в цепи есть», а не «цепь подтвердила
 * формулировку дословно».
 *
 * ⚠️ Здесь стояло «и цепь различить их постфактум не может» — с 21 августа 2026
 * (пункт 101) это половина правды: `overturnedVerdicts` растёт только на двух
 * путях переворота и таймаут обходит, значит различить цепь МОЖЕТ. А эта дверь
 * его всё равно не читает — у пары «чистые/перевёрнутые» нет ни порогов, ни
 * последствий по решению владельца. Так что вывод про `verifiedByChain`
 * прежний, а довод к нему теперь другой.
 */
