...
export default {
  /**
   * ⚠️ АВТОМАТИЧЕСКИЙ JSX-РАНТАЙМ — КАК В NEXT, а не как по умолчанию у esbuild.
   *
   * По умолчанию esbuild компилирует JSX в `React.createElement`, то есть в
   * обращение к свободной переменной `React`. Next собирает автоматическим
   * рантаймом (`jsx-runtime`), поэтому файлы страниц React не импортируют — и
   * не обязаны. Под тестами такой файл падал на рендере: «React is not
   * defined» (замерено на `app/chat/page.tsx`).
   *
   * То есть без этой строки тестировать РАЗМЕТКОЙ можно было только те
   * компоненты, которые случайно импортируют React сами, — а именно разметкой в
   * этом проекте ловятся самые дорогие промахи (находка К-2: разводка причин
   * доехала до панели и не доехала до списка). Компонентам, которые React
   * импортируют, автоматический рантайм не мешает ничем.
   */
