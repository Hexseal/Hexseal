...
//
// Заменяет ArbiterRegistryFacet в Diamond: applyAsArbiter() теперь тянет
// ARBITER_BOND (50 USDC) с кандидата, форфейтится в vaultBalance при
// демоушене. Новый resignAsArbiter() — самостоятельный выход с возвратом
// бонда, но блокируется пока у арбитра есть открытый dispute claim
// (openClaimCount) — иначе бонд можно было бы забрать, бросив живой спор.
//
// Replace: все существующие 35 селекторов (whole-facet redeploy, см.
//   UpgradeArbiterRegistryFacetDemotion.s.sol).
// Add:     resignAsArbiter, getArbiterBond, getOpenClaimCount
//
// Применять вместе с UpgradeReputationFacetCounterpartyGate.s.sol.
//
// Usage:
//   forge script script/UpgradeArbiterRegistryFacetBondAndGuard.s.sol \
...
contract UpgradeArbiterRegistryFacetBondAndGuard is Script {
    ...
    function run() external {
        ...
        replaceSels[4]  = ArbiterRegistryFacet.claimDispute.selector;
        ...
        replaceSels[34] = ArbiterRegistryFacet.getArbiterMistakeStreak.selector;

        bytes4[] memory addSels = new bytes4[](3);
        addSels[0] = ArbiterRegistryFacet.resignAsArbiter.selector;
        addSels[1] = ArbiterRegistryFacet.getArbiterBond.selector;
        addSels[2] = ArbiterRegistryFacet.getOpenClaimCount.selector;
        ...
        require(
            IDiamondLoupe(DIAMOND).facetAddress(ArbiterRegistryFacet.resignAsArbiter.selector) == address(facet),
            "resignAsArbiter: wrong facet"
        );
        require(
