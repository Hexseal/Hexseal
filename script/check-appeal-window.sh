...
# I3 (закрывающий раунд ревью): APPEAL_REVIEW_WINDOW_DAYS — плоская `const`
# в bagStore.js, не env-конфигурируемая ручка (была ею до находки координатора:
# `APPEAL_REVIEW_WINDOW_DAYS=1` в process.env давало срок на три дня короче
# нужного, а этот гейт — статический, сверяет ИСХОДНИК, не то, чем реально
# запущен процесс — оставался зелёным, потому что проверял только позитивность
# числа, не равенство четырём). Якорь `^export const ... = \K[0-9]+` —
# ТОЧНОЕ совпадение строки объявления, `^` не даёт спутать её с прозой в
# комментариях выше по файлу, упоминающих то же имя тем же символом "=".
