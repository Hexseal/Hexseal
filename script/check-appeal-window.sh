...
# docs/OPEN-ITEMS.md, пункт 29, «Мелочь оттуда же» (ссылка по номеру пункта, а
# не по строкам: строки уезжают при любой правке реестра).
# `|| node_exit=$?` — стандартный приём:
