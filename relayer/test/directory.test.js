...
describe('directory.js — форма и хранение', () => {
  ...
  // регистрация раньше ВСЁ РАВНО переписывала весь файл целиком, а клиент
  // чата шлёт свой ключ при КАЖДОМ запуске сеанса (lib/chatSession.ts,
  // Задача 4) — то есть обычное пользование усиливало открытый пункт 31
  // (docs/OPEN-ITEMS.md; цена записи там намеренно не публикуется до
  // починки), без единого нападающего. Тест выше проверяет ТОЛЬКО итог
  // (history/keyChangeCount/
...
// ─── В-2, ОБРАТНОЕ НАПРАВЛЕНИЕ (сквозная проверка, 8 августа) ─────────────
//
// Тесты выше моделируют одну сторону: «другая копия схлопнула, мы дописываем».
// Обратной стороны — «схлопываем МЫ, а чужая запись лежит ТОЛЬКО в журнале» —
// не было ни одной, и ровно в это слепое место въехала регрессия внутри одной
// волны правок: коммит про журнал отменил защиту, написанную днём раньше.
//
// Замер на двух настоящих процессах поверх одного хранилища (боевые
// умолчания): адрес, зарегистрированный другой копией, после нашего
// схлопывания ПОТЕРЯН, а keyChangeCount у общего адреса откатился 1 → 0 —
// хотя модуль объявляет его никогда не убывающим и стережёт это в двух
// других местах. 826 тестов при этом оставались зелёными.
//
// Почему так вышло: _saveDirectory() сливает с диском только СНИМОК
// (_mergeWithDisk), журнал не переигрывает — _replayDirectoryJournal зовётся
// лишь из _loadDirectory — и затем безусловно сносит журнал. Всё, что чужая
// копия дописала после нашей загрузки, исчезает вместе с ним.
describe('В-2, обратное направление: схлопываем МЫ, чужая запись только в журнале', () => {
  it('адрес, дописанный другой копией в журнал, переживает НАШЕ схлопывание', () => {
    putKey(ALICE, { boxKey: KEY_A }, 1000);

    // «Другая копия» дописала CAROL строкой в журнал и снимка не трогала —
    // ровно то, что делает putKey() после перехода на журнальную запись.
    fs.appendFileSync(
      directory.DIRECTORY_FILE + '.log',
      JSON.stringify({
        a: CAROL,
        r: { v: 1, boxKey: KEY_C, updatedAt: 1500, history: [], keyChangeCount: 0 },
      }) + '\n',
      'utf8',
    );

    // Схлопывание. Это ровно та функция, которую зовёт putKey() при переходе
    // журнала через потолок; зовём её прямо, чтобы не гонять четыре мегабайта.
    _saveDirectory();

    const after = directoryOnDisk();
    expect(after[CAROL], 'запись чужой копии стёрта нашим схлопыванием').toBeDefined();
    expect(after[CAROL].boxKey).toBe(KEY_C);
    expect(after[ALICE].boxKey).toBe(KEY_A);

    // И главное — она обязана пережить перезапуск: журнал после схлопывания
    // снесён, значит уцелеть запись может только в самом снимке.
    _loadDirectory();
    expect(getKeyRecord(CAROL), 'после перезапуска запись чужой копии потеряна').not.toBeNull();
  });

  it('счётчик смен не убывает и когда чужая смена лежит только в журнале', () => {
    putKey(ALICE, { boxKey: KEY_A }, 1000);   // наш счётчик: 0

    // Другая копия провела смену ключа тому же адресу — строкой в журнал.
    fs.appendFileSync(
      directory.DIRECTORY_FILE + '.log',
      JSON.stringify({
        a: ALICE,
    ...
    const after = directoryOnDisk();
    expect(after[ALICE].keyChangeCount, 'улика откачена назад нашим схлопыванием').toBe(1);
  });
});
