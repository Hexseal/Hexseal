 ...
 *
 * ⚠️ РЕВЬЮ КРУГ 2 -> КРУГ 3, ИСПРАВЛЕНО. Круг 2 предложил три средства против
 * дешёвого спама через опрос: сжать бюджет опроса (9→4), завести короткий
 * отрицательный кэш, поставить ограничитель по IP (на живом Next-пути, не
 * здесь — здесь IP-ключ уже стоял, app.js:checkRateLimit по clientIp()).
 * Круг 3 отменил первые два: бюджет ВОЗВРАЩЁН к 9 (сжатие опиралось на цифру
 * из тестовой СЦЕНЫ «отстаёт», не на замер — настоящий замер лага реплик,
 * `frontend/src/lib/walletLock.ts:166-172`, даёт трёхкратный запас именно у
 * 9×750); отрицательный кэш УБРАН целиком (он переносил неудачу ПЕРВОГО
 * спросившего на ЛЮБОГО другого, включая контрагента по той же сделке).
 * Единственная причина заводить кэш (дешевизна спама) закрыта оставшимся
 * IP-ограничителем и без кэша: 30/мин × 9 = 270 чтений/мин с одного
 * источника — не то число, которое в этом проекте кого-то пугало.
 * НАХОДКА 2 круга 2: подпорка `client !== ZERO_ADDR` убрана — она была
 ...
 * `замолкаетПослеПервого` — ПЕРВОЕ чтение разбирается и отдаёт пустую запись
 * («не наш» отставшей реплики), все последующие БРОСАЮТ: узел мигнул ровно
 * посреди опроса. Смешанный случай итогового ревью ветки — сцена 8.
 */
function поднятьЦепь({
  ...
  mockContract(DIAMOND, {
    getRecord: async (addr) => {
      ...
      if (замолкаетПослеПервого) {
        if (счёт.чтенийРеестра > 1) throw new Error('узел замолчал посреди опроса (test)');
        return ПУСТАЯ;
      }
      if (отстаётПопыток > 0) {
...
describe('Пункт 44 (релеер): платим газ только за свои контракты', () => {
  ...
  it('ЗАМЕР «долбят нарочно»: сто отказанных запросов — ноль транзакций и ноль газа', async () => {
    ...
    expect(счёт.отправок).toBe(0);
    expect(счёт.нонсов).toBe(0);
    // Цена спама после замка: attempts чтений цепи за запрос (не одно) и
    // НОЛЬ газа. Без отрицательного кэша (убран на круге 3) каждый запрос
    // платит полный опрос заново — кому больно, ограничивает не кэш, а
    // IP-лимитер на живом Next-пути (здесь, в релеере, — checkRateLimit по
    // clientIp(), 10/мин).
  ...
  it('смешанный случай, где узел ОТВЕТИЛ хоть раз: «не наш» подтверждён живым узлом — 403, не 503', async () => {
    ...
    expect(res.status).toBe(403);
    expect(res.body.code).toBe('target_not_ours');
    expect(счёт.отправок).toBe(0);
