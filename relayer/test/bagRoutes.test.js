...
describe('POST /bags/pass', () => {
  ...
  it('лимитер по заявленному адресу срабатывает ДО восстановления подписи (проверка порядка — дешёвое раньше дорогого)', async () => {
    // Если бы порядок был обратным (сначала подпись, потом лимитер), все 11
    // запросов с негодной подписью получили бы 401 invalid_signature и ни
    // один — 429, потому что до лимитера код никогда бы не доходил.
    const { address } = await newWalletAndAddress();
    const ts = Math.floor(Date.now() / 1000);
    let last;
    for (let i = 0; i < 11; i++) {
      last = await request(app)
        ...
        .set('x-sig', '0xnotasignature')
        .send({ address });
      if (i < 10) expect(last.status).toBe(401);
    }
