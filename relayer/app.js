...
// замера — docs/superpowers/specs/2026-08-02-e2e-chat-design.md §4):
//
...
//
// Настоящий корень задачи не здесь: кошельки не сверяют имя домена с адресом
// сайта, поэтому выведенный ключ предъявительский (спека
// `docs/superpowers/specs/2026-08-02-e2e-chat-design.md` §4).
//
// ⚠️ РАЗВИЛКА ЖИВЁТ В ОТДЕЛЬНОМ ДОКУМЕНТЕ — `docs/OPEN-QUESTION-phishing.md`.
// Читать ЕГО, а не пересказ здесь: там разобраны и пароли-ключи (WebAuthn с
// постоянным секретом — навязывают привязку силой, но требуют решить
// восстановление истории), и предложенный владельцем третий путь — вывод ключа
// внутри НАШЕГО собственного окна плюс секрет, который сервер отдаёт только
// настоящему браузерному запросу с нашего домена. Там же названо, где третий
// путь ломается: подделка со своим сервером проходит. Здесь этот выбор не
// делается и делаться не должен.
//
...
app.put('/disputes/:agreement/bags', async (req, res) => {
  ...
  // ⚠️ 4, а не 3. У реестра DISPUTED = 3, у самой сделки = 4 — два разных
  // enum. Спутав их, замок принял бы завершённую сделку за спорную.
  if (!facts.disputed) {
...
app.get('/disputes/:agreement/bags', async (req, res) => {
  ...
  res.json({ bags, arbiter: facts.arbiter, sealedForOthers });
});
...
app.get('/disputes/:agreement/bags/:name', async (req, res) => {
  ...
  //
  // ⚠️ И ЕЩЁ ЧЕСТНЕЕ: markFetched переводит мешок с 30-дневного срока «не
  // прочитан» на 7-дневный «прочитан» (bagExpiryAt), а спор с апелляцией
  // живёт ≈9 дней и верхней границы не имеет вовсе. То есть после первого
  // захода арбитра предъявление может умереть РАНЬШЕ спора. Здесь это не
  // чинится — это Задача 2 плана 4в-2, и она начинается отсюда.
