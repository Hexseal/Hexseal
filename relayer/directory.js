...
// миг, всё ещё могут потерять работу друг друга. Настоящее решение —
// файловая блокировка или один-единственный писатель, и это отдельная задача.
//
// ⚠️ Про размер окна здесь раньше стояло «сузилось до одного чтения-записи».
// Это было неверно и замерено: после перехода на журнальную запись окно
// равнялось ВСЕМУ, что чужая копия успела записать с момента нашей загрузки
// (в замере — 1028 записей), потому что _saveDirectory сливала только снимок
// и сносила журнал. Теперь журнал доигрывается перед сносом, и окно
// действительно сузилось до одновременной записи.
...
export function _saveDirectory() {
  ...
  try {
    ...
    // ...и доигрываем журнал ТУДА ЖЕ, перед тем как его снести.
    //
    // Слияния со снимком мало. С переходом на журнальную запись чужая копия
    // больше НЕ пишет снимок при каждой регистрации — она дописывает строку
    // в журнал. Значит всё, что она зарегистрировала после нашей загрузки,
    // живёт только там, и снос журнала ниже уносил это с собой безвозвратно.
    //
    // Замер на двух настоящих процессах поверх одного хранилища: адрес чужой
    // копии терялся целиком, а keyChangeCount общего адреса откатывался
    // 1 -> 0 — при том, что модуль объявляет его никогда не убывающим и
    // стережёт это в двух других местах (_replayDirectoryJournal,
    // _mergeWithDisk). Файл сам про себя пишет: «this file IS the data» —
    // восстановить потерянное неоткуда.
    //
    // Порядок именно такой: снимок, потом журнал. Журнал новее снимка по
    // построению, и его записи обязаны победить; сам _replayDirectoryJournal
    // при этом берёт БОЛЬШИЙ счётчик, так что улика не убывает и здесь.
    _replayDirectoryJournal(_directory);
