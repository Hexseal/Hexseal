...
contract ArbiterAccountabilityFacet {
    ...
    // ArbiterRegistryStorage.clearSeat — ОДНОЙ точке на все двери выхода из
    // корпуса, а не только здесь. Обоснование то же, что и для
    // removeArbiterForCause: предложение не должно пережить человека, против
    // которого оно висело.
    //
    // ⚠️ ПОИМЁННОГО СПИСКА ЗДЕСЬ БОЛЬШЕ НЕТ, И ЭТО НАМЕРЕННО. Он стоял, он
    // протух и он лгал: до задачи 12 (18 августа 2026) третьей дверью
    // называлась `_recordArbiterMistake`, а она с тех пор кресла не отнимает
    // вовсе — третья ошибка приостанавливает и обвиняет. Поверивший списку
    // сделал бы ВЫВОД, ОБРАТНЫЙ тому, ради которого задача 12 и делалась.
    //
    // Вместо списка — свойство и способ его проверить: `clearSeat` зовут
    // только двери выхода, и проверяется это `grep -rn "clearSeat(" src/`.
    // Актуальный перечень вызывающих держит докстринг самой `clearSeat`, в
    // одном месте, а не в трёх копиях по двум файлам.
    //
