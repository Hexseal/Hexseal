...
contract ArbiterAccountabilityFacet {
    ...
    /// только владелец. Отдельная ошибка, а не NotOwner: директор, получивший
    /// NotOwner на функции, которая ему в общем случае разрешена, пошёл бы
    /// искать проблему в своей роли. Здесь дело не в роли, а в весе конкретной
    /// приостановки.
    error RemovalSuspensionIsOwnerOnly();
