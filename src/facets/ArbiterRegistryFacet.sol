...
library ArbiterRegistryStorage {
    ...
    ///
    /// ⚠️ `suspendedUntil` стирается здесь же (финальный обзор ветки, M-4,
    /// 16 августа 2026). Снос теперь ВЫСТАВЛЯЕТ приостановку
    /// (ArbiterAccountabilityFacet.removeArbiterForCause, C-1), а не стирал её
    /// до этой правки НИКТО: ни одна дверь выхода, ни повторная посадка.
    /// Владелец, чинящий ошибочный снос одной командой addArbiter, возвращал бы
    /// человека с недожитой приостановкой — тот молча не может ни клеймить, ни
    /// финализировать, ни уволиться, и почему, из цепи не видно ничем, кроме
    /// getSuspendedUntil. Ровно то же правило, что и для двух полей выше:
    /// признаки прошлого сноса не переживают повторную посадку.
    ///
    /// ⚠️ ЗНАЕМЫЙ ШОВ, решение о поведении за владельцем: вторая дверь входа —
    /// applyAsArbiter — самозапись, и снятый по поводу, если ДАО активна и у
    /// него хватает XP/серии на новый залог, СНИМАЕТ ЭТИМ СВОЮ ЖЕ приостановку
    /// и может финализировать вердикты, взятые до сноса, не дожидаясь окна.
    /// Цена обхода — свежие 50 USDC залога (ARBITER_BOND) поверх только что
    /// сожжённого, и до активации ДАО путь мёртв целиком (applyAsArbiter
    /// ревертит DAONotActive). Разделять двери здесь не стали: это вернуло бы
    /// две копии уборки, ровно тот дефект, ради которого функция и вынесена в
    /// библиотеку. Записано в отчёте финального обзора.
    function clearRemovalRecord(Data storage d, address arbiterAddr) internal {
        ...
        delete d.suspendedUntil[arbiterAddr];
    }
}
