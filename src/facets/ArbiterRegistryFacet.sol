...
library ArbiterRegistryStorage {
    ...
    struct Data {
        ...
        /// AUTO_REMOVAL_CODE рядом с функциями записи): 0 — не снимали ни разу,
        /// 1..6 — код Cause плюс единица, AUTO_REMOVAL_CODE — автодемоушен, у
        /// которого повода нет вовсе. Тип uint8, а не enum: тот объявлен в
    ...
    /// Код автодемоушена. Повода у него нет: цепь сняла арбитра по серии
    /// ошибок, а не по чьему-то обвинению, и запись обязана говорить это
    /// прямо, а не притворяться поводом номер ноль. Значение взято на другом
    /// конце диапазона, чтобы никакой будущий Cause в него не дорос.
    uint8 internal constant AUTO_REMOVAL_CODE = 255;
    ...
    /// Автодемоушен. Отдельная точка входа, а не «передай сюда 255»: если бы
    /// код называл вызывающий, кодировка получила бы второго хозяина — и
    /// разошлась бы при первой же правке.
    function recordAutomaticRemoval(Data storage d, address arbiterAddr) internal {
        _recordRemoval(d, arbiterAddr, AUTO_REMOVAL_CODE);
    }

    /// ⚠️ Зовётся из ветки, которую Agreement исполняет внутри пустого
    /// try/catch (src/Agreement.sol:964, notifyArbiterTimeout). Ревертить здесь
    /// нельзя: ревёрт был бы проглочен молча, и арбитр остался бы ненаказанным
    /// без единого следа. Ни одна строка ниже ревертить не может — инкремент
    /// uint256 и две записи.
...
contract ArbiterRegistryFacet {
    ...
    /// Директор не может собрать блок, РЕШАЮЩИЙ апелляцию: иначе его
    /// ставленники определяют исход любой апелляции, а после передачи сноса
    ...
    function _recordArbiterMistake(
        ...
        if (mistakes >= MAX_ARBITER_MISTAKES) {
            ...
            // Вечная половина той же записи (п. 72): removedAt выше стирается
            // повторной посадкой, эта — нет.
            ArbiterRegistryStorage.recordAutomaticRemoval(d, arbiterAddr);
