...
contract Treasury {
    ...
    function distribute() external nonReentrant {
        ...
        // только один из них — ДО вызова банка. Из-за этого реентрант,
        // попавший сюда изнутри _fundVault() (через raw CALL к диамонду),
        // читает pendingDistribution() == 0: earmarked уже поднялся ровно на
        // rest, а токены, которые банк успел стянуть через transferFrom, уже
        // списаны с баланса. Второй раз распределять ему нечего — денежный
        // инвариант держится САМ ПО СЕБЕ, без какого-либо стража. Проверено
        // ревью на текущей версии контракта прогоном двух атакующих
        // (обычный сплит и пост-ДАО): в обоих случаях без nonReentrant
        // баланс не расходится ни на копейку.
        //
    ...
    /// недостающую сумму. Ни привилегированного вызывающего, ни выбора суммы,
    /// ни выбора момента — всё определяется состоянием. Никто не «решает»
    /// потратить резерв: решает правило.
    ///
