...
// может быть строже его владельца. Ревью нашло два действующих (не
...
//       селекторов нет.
//
//   (b) FactoryFacet.setRegionFee(region, 0) (onlyOwner, без нижней
//       границы, FactoryFacet.sol:249-253) — обнуляет единственную
//       реальную стоимость накрутки счётчика честными сделками.
//       ReputationFacet начисляет uniqueActiveUsers на каждый свежий адрес
//       при завершённой сделке от 10 USDC (ReputationFacet.sol:308-312), по
//       два уникальных адреса на сделку — 50 000 сделок на весь порог.
//       Полный цикл сделки — 1 131 280 газа; итог — 3-17 тыс. долларов
//       против 192 тыс. долларов резерва, и капитал не расходуется:
//       эскроу-депозит рециклируется между сделками.
//
...
// diamondCut-события; (b) стоит на два порядка меньше самого резерва и
// маскируется под обычный поток сделок. Ни то ни другое казна предотвратить
// не может — она читает состояние диамонда, а не контролирует его.
// ============================================================
...
contract Treasury {
    ...
    /// недостающую сумму (min(просадка, резерв) в блоке вызова) — вызывающий
    /// выбирает МОМЕНТ (функция открытая) и тем самым косвенно влияет на
    /// сумму (вызов сразу после крупной выплаты арбитру двигает больше, чем
    /// вызов до неё), но этот выбор ему НИЧЕГО не даёт: получатель перевода,
    /// его назначение и правило расчёта суммы от вызывающего не зависят.
    ///
    ...
    /// ОБЯЗАНА ОКАЗАТЬ») про два действующих пути того же владельца обойти и
    /// этот гейт.
