...
contract Treasury {
    ...
    function distribute() external nonReentrant {
        ...
        // БЕЗ nonReentrant этот страж защищает ДЕНЬГИ, не только телеметрию.
        // Измерено ревью (атакующий реентерит distribute() из fundVault() ДО
        // собственного pull, а на самом глубоком уровне отчитывается о
        // полном банке): вложенный вызов, увидевший схлопнувшуюся
        // vaultShortfall() == 0, размечает СТО ПРОЦЕНТОВ своего pending в
        // reserveBalance/foundationOwed — включая деньги, которые уровень
        // выше УЖЕ разрешил (approve) к списанию и которые диамонд стянет
        // сразу после возврата из этого вложенного вызова. Замерено на
