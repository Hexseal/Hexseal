...
contract ArbiterSuspensionTest is Test, ArbiterTwoFacetBench {
    ...
    // ============================================================
    //  АВТОМАТИЧЕСКИЙ ПУТЬ ТОЖЕ ПРИОСТАНАВЛИВАЕТ (правка A, 16 августа 2026)
    //
    ...
    //  смотрит на приостановку, а не на статус, так что наказанный доводил уже
    //  взятые споры до денег внутри FINALIZE_DELAY, и остановить его было
    //  нечем (suspendArbiter на неарбитре ревертит NotAnArbiter).
    //
    //  ⚠️ ЗАДАЧА 12 (18 августа 2026) СДЕЛАЛА ЭТУ ПРИОСТАНОВКУ ГЛАВНЫМ, А НЕ
    //  ПОБОЧНЫМ ДЕЙСТВИЕМ ветки порога: снятия там больше нет вовсе, есть
    //  обвинение от имени цепи и 48 часов паузы. Значит все двое суток между
    //  ошибкой и сносом деньги держит ровно эта отметка и ничто другое.
    //
    //  Оба теста доказывают ПОСЛЕДСТВИЕМ — отказом finalizeVerdict, — а не
    //  чтением поля: поле можно выставить и не читать нигде.
    // ============================================================

    /// Доводит `arbiter` до порога автоматики настоящим путём: три перевёрнутых
    ...
    function test_AutoDemotedArbiterCannotFinalize() public {
        ...
        // Спор, взятый ДО приостановки: его-то остановленный и доводил бы до денег.
        ...
        // ⚠️ ЗАДАЧА 12 (18 августа 2026): третья ошибка больше НЕ снимает —
        // она приостанавливает и обвиняет от имени цепи. Кресло переживает, и
        // сцена от этого стала острее, а не слабее: доказываемое свойство —
        // «автоматический путь ставит ту же приостановку, и она держит деньги»
        // — теперь единственное, что стоит между человеком и котлом на все 48
        // часов паузы. Проверяется ПОСЛЕДСТВИЕМ, отказом finalizeVerdict ниже,
        // а не чтением поля.
        assertTrue(reg.isRegisteredArbiter(arbiter), unicode"сцена не та: кресло обязано пережить автоматический путь");
