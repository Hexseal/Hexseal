...
// Казна встанет на место feeRecipient и должна иметь право наполнять
// банк арбитров через fundVault. Проверяем, что это право есть у владельца
// и у текущего получателя комиссий, но не у постороннего адреса.
...
contract FundVaultAccessTest is Test {
    ...
    function setUp() public {
        ...
        bytes4[] memory arbSels = new bytes4[](26);
        ...
        arbSels[25] = ArbiterRegistryFacet.getVaultBalance.selector;
    ...
    /// Получатель комиссий — это адрес, которым станет казна. Без права
    /// пополнить банк арбитров она не сможет исполнять первую ступень лестницы.
