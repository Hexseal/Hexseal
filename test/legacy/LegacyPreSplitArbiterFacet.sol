...
// ════════════════════════════════════════════════════════════════════════
// ЗАМОРОЖЕННЫЙ СЛЕПОК ВЫКАЧЕННОГО ФАСЕТА — НЕ ПРАВИТЬ (уборка 7а, п. 5)
// ════════════════════════════════════════════════════════════════════════
//
// ЧТО ЭТО. Дословная копия `src/facets/ArbiterRegistryFacet.sol` с коммита
// `b110fae1` (=`main`, =`origin/main`) — то есть ТОГО САМОГО исходника, из
// которого собран фасет, задеплоенный на Base Sepolia 15 августа 2026
// (`0x1CF4c7DaA27f2241eafd8E818329719418403013`, 64 арбитражных селектора).
// Изменены только имена, чтобы копия ужилась в одном проекте с оригиналом:
...
//   import "../RegistryFacet.sol" → "../../src/RegistryFacet.sol" (файл переехал
//     на два уровня, путь пересчитан)
// Ни одна строка ЛОГИКИ не тронута. Проверяется diff'ом против `git show`.
//
// ЗАЧЕМ. Стенды исполненных разрезов (`test/ArbiterChatKeyUpgrade.t.sol`,
// `test/PresentationRecordUpgrade.t.sol`) обязаны воспроизводить раскладку
// цепи НА ТОТ МОМЕНТ: все арбитражные селекторы на ОДНОМ адресе, и адрес
// обязан на них РЕАЛЬНО отвечать — пред-полёт тех скриптов зовёт
// getArbiters()/getVaultBalance()/getArbiterFloor()/getOpenClaimCount
// по-настоящему, а `checkReplaceGroup` требует единого адреса на всю группу.
//
// ПОЧЕМУ СЛЕПОК, А НЕ НАСЛЕДНИК. До этой правки двойник объявлялся как
// `contract LegacyPreSplitArbiterFacet is ArbiterRegistryFacet` и дописывал
// четырнадцать чтений, уехавших задачей 4.5. То есть он НАСЛЕДОВАЛ наш
// сегодняшний код и ехал за каждой его правкой — при том что заявлен был как
// «раскладка цепи». Две беды сразу:
//   • он рос вместе с нами и 16 августа перевалил EIP-170 (24 646 → 24 722 при
//     пределе 24 576): `forge build --sizes` отвечал кодом 1. Раскладка,
//     которую НЕЛЬЗЯ задеплоить, цепь не описывает по определению;
//   • он показывал стендам сегодняшнее поведение под видом вчерашнего —
//     молча, и чем дальше, тем сильнее.
// Слепок обе беды снимает по построению: 21 227 байт (запас 3 349), и наши
// правки в `src/` его не двигают вовсе. Замер — в отчёте задачи 7а.
//
// ⚠️ ЭТОТ ФАЙЛ НЕ ПРАВЯТ. Он описывает ПРОШЛОЕ. Если понадобится слепок
// следующего выкаченного состояния — заводится ВТОРОЙ файл рядом, со своим
// коммитом в шапке, а этот остаётся как есть. Дублирование библиотеки
// хранилища здесь не небрежность, а условие заморозки: подтянуть её из `src/`
// значило бы снова ехать за нашими правками (например, задача 7а поменяла
// подпись `recordAutomaticRemoval` — со ссылкой на живую библиотеку копия бы
// уже не собралась).
//
// ⚠️ В `src/` его нет и быть не должно: он существует, чтобы прошлое можно
// было воспроизвести, а не чтобы у чтений стало два дома.
//
// ════════════════════════════════════════════════════════════════════════
// Ниже — исходный заголовок файла на момент снимка, как он был.
// ════════════════════════════════════════════════════════════════════════
...
contract LegacyPreSplitArbiterFacet {
    ...
    function claimDispute(
        ...
        // поля в LegacyArbiterRegistryStorage.
