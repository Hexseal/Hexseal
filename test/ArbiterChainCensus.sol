...
abstract contract ArbiterChainCensus is CommonBase {
    ...
    function _censusFromFile(
        ...
        // правило не распространяется, и остаётся литерал.
        //
        // ⚠️ ДОВОД ЗДЕСЬ БЫЛ ЛОЖНЫЙ, И ЭТО ОПРОВЕРГНУТО ЗАМЕРОМ (переревью,
        // 17 августа 2026). Прежняя редакция объясняла литерал тем, что
        // спросить имя чужого скрипта нельзя без `new`, а лишний `new` — это
        // гонка nonce. Неверно дважды:
        //
        //   • `new UpgradePresentationRecord().scriptPath()` ВНУТРИ сверки
        //     гонки не заводит вовсе: `setUp` выполняется перед КАЖДЫМ тестом
        //     заново, состояние между тестами Foundry откатывает, и `new` в
        //     теле теста физически не может сдвинуть адреса, посчитанные в
        //     `setUp` до него. Замерено: 0 красных;
        //   • а файловая `string constant` в самом скрипте отдаёт имя вообще
        //     без развёртывания — 859 зелёных.
        //
        // Убедительно звучащий ложный довод, записанный в код, вводит
        // следующего читателя в заблуждение вернее, чем отсутствие довода, —
        // поэтому он заменён, а не подпёрт.
        //
        // НАСТОЯЩАЯ ПРИЧИНА ОСТАВИТЬ ЛИТЕРАЛ: выгода нулевая. Замок на своём
        // скрипте работает потому, что у следующего разреза скрипт ДРУГОЙ, —
        // копипаста стенда тащит вызов, а отвечает он уже новым именем. С
        // чужим скриптом этого не происходит: и `import` типа, и строка в
        // кавычках переезжают копипастой ОДИНАКОВО МОЛЧА и обе продолжают
        // называть прежний разрез. Платить за ту же защищённость лишней
        // зависимостью стенда от чужого скрипта незачем.
        require(
