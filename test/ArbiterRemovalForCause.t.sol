...
contract ArbiterRemovalForCauseTest is Test {
    ...
    ///
    ///   • а вот ТРЁХ РАЗНЫХ СПОРОВ не требуется, и это стоит сказать прямо:
    ///     три отмены ОДНОГО агримента проходят и снимают арбитра — замерено
    ///     живым тестом ArbiterRemovalForCauseIntegration::
    ///     test_ArbiterDemotedNamesOwnerOnTheOverturnPath.
    ///
    ...
    /// первого пути — в докстринге того теста: он требует живого
    /// нефинализированного вердикта этого арбитра, а трёх РАЗНЫХ споров не
    /// требует. Безусловная запасная дверь — `diamondCut`. Замок глушит
