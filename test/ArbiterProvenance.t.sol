...
contract ArbiterProvenanceTest is Test, ArbiterTwoFacetBench {
    ...
    /// Тот же директор-арбитр, но посадивший себя САМ. Блок — это множество
    /// голосующих, а не сумма двух счётчиков: `seatedCountBy[chief]` его уже
    /// посчитал, и слепое «плюс он сам» назвало бы одного человека двумя
    /// голосами.
    ///
    /// Что исчезнет из поведения, если снять правку: `getChiefBloc()` начнёт
    /// отвечать 2 при одном реальном голосе. Двойка — это порог, за которым
    /// апелляция считается решённой; читатель геттера (сегодня — человек,
    /// завтра — экран) принимает по этому числу решение, и врать ему на
    /// единицу нельзя.
    ///
    /// Отказ второй посадки при этом НЕ доказательство правки: он одинаков в
    /// обеих редакциях (2 против 3, обе >= 2). Поэтому проверяется само число,
    /// а порог — отдельной строкой ниже, чтобы «единица» не оказалась
    /// случайным совпадением с «блок пуст».
    function test_ChiefWhoSeatedHimselfIsCountedOnce() public {
        vm.prank(chief);
        facet.addArbiter(chief);            // директор сажает САМ СЕБЯ

        assertTrue(facet.isRegisteredArbiter(chief), unicode"сетап: директор сел");
        assertEq(accFacet.getSeatedBy(chief), chief, unicode"сетап: посадил себя он сам");
        assertEq(accFacet.getSeatedCountBy(chief), 1, unicode"сетап: счётчик посадок засчитал его");

        assertEq(
            facet.getChiefBloc(), 1,
            unicode"один человек — один голос: слагаемые пересеклись, а не сложились"
        );
    }
    ...
    ///
    /// ⚠️ Длина проверяется ЯВНО (уборка 7а, М-4 ревью задачи 3). Без проверки
    /// `err.length - 4` на коротком возврате — Panic(0x11), арифметическое
    /// переполнение uint256, из которого читатель узнаёт ровно ничего: ни того,
    /// что ревёрт пришёл не тот, ни того, каким он был. А короткий возврат тут
    /// не выдумка: пустая строка приезжает от `require` без сообщения, от
    /// out-of-gas и от вызова в адрес без кода — то есть ровно в тех случаях,
    /// когда ошибиться легче всего и разобраться нужнее всего.
    function _stripSelector(bytes memory err) internal pure returns (bytes memory out) {
        require(
            err.length >= 4,
            unicode"_stripSelector: возврат короче селектора — ревёрт пришёл не тот"
        );
